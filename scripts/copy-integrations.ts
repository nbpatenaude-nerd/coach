...
async function main() {
  ...
  const streamFrom = prod.query(copyTo(`COPY (SELECT ${colString} FROM "Integration") TO STDOUT`));
  const streamTo = staging.query(copyFrom(`COPY "Integration" (${colString}) FROM STDIN`));
