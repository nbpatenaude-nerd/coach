...
description: Use Journey Endurance with Cursor and other AI tools through the Model Context Protocol (MCP) server.
---

Journey Endurance includes a remote **MCP server** that lets supported AI assistants access your training data securely. Instead of copying workout details into a chat window, your assistant can read your plan, log nutrition, and help manage your schedule directly.
...
2. Add the Journey Endurance server:

```json
{
  "mcpServers": {
    "coach-watts": {
      "url": "https://journeyendurance.ca/mcp"
    }
  }
}
```
...
4. When Cursor connects, your browser opens the Journey Endurance sign-in and consent screen.
5. Review the requested permissions and click **Allow**.

::alert{type="success"}
Success: After authorization, Cursor can call Journey Endurance tools on your behalf. Try asking it about your upcoming workouts or recovery status.
::
...
1. Go to **Settings → Apps** in Journey Endurance.
...
- **Use the in-app coach too** — MCP and the Journey Endurance chat share the same underlying data. Use whichever interface fits the moment.

## Troubleshooting

### Cursor shows "Unauthorized" or won't connect

- Sign out and back in to Journey Endurance, then try connecting again.
...
- Verify the data exists in your Journey Endurance dashboard first.
