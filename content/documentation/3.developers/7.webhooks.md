---
title: Webhooks
description: Push data to Journey Endurance via incoming webhooks for OAuth applications.
---

OAuth applications can push data to Journey Endurance using **incoming webhooks**. This is useful when your integration receives data from external services and needs to forward it to Journey Endurance without making direct API calls.
...
```
POST https://journeyendurance.ca/api/webhooks/oauth/{clientId}
```
...
Send a `POST` request with a JSON body. Journey Endurance captures the raw payload and associates it with your application for asynchronous processing.

```bash
curl -X POST "https://journeyendurance.ca/api/webhooks/oauth/YOUR_CLIENT_ID" \
