    ...
    <template #body>
      <div class="max-w-3xl mx-auto p-4 sm:p-8 space-y-8 pb-24">
        <div class="text-center space-y-4">
          <div class="w-16 h-16 bg-primary-50 dark:bg-primary-900/20 rounded-2xl flex items-center justify-center mx-auto border border-primary-100 dark:border-primary-800">
          ...
          <p class="text-sm text-gray-600 dark:text-gray-400 max-w-md mx-auto leading-relaxed">
            Upload CSV data from external sensors (Moxy, EIR, etc.). Map the columns to your Coach Watts timeline to merge them into your workouts.
          </p>
        </div>

        <UCard :ui="{ root: 'rounded-xl shadow-lg border-gray-100 dark:border-gray-800', body: 'p-6' }">
          
          <!-- Step 1: Select File -->
          <div v-if="!fileParsed" class="space-y-4">
            <UFormGroup label="Select CSV File" description="Must contain a header row with column names.">
              <input ref="fileInput" type="file" accept=".csv" class="hidden" @change="handleFileSelect" />
              <div class="flex gap-4 items-center">
                <UButton color="primary" variant="outline" icon="i-heroicons-document-arrow-up" @click="() => fileInput?.click()">
                  Choose File
                </UButton>
                <span class="text-sm font-medium text-gray-500">{{ selectedFile?.name || 'No file selected' }}</span>
              </div>
            </UFormGroup>
          </div>
          ...
          <div v-else class="space-y-6">
            ...
            <p class="text-sm text-gray-500 mb-4">Select which CSV columns correspond to our tracked metrics. Time is required.</p>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <UFormGroup v-for="field in standardFields" :key="field.id" :label="field.label" :required="field.required">
                <USelect
                  v-model="mapping[field.id]"
                  :options="[{ label: 'Ignore (None)', value: '' }, ...csvColumns.map(c => ({ label: c, value: c }))]"
                />
              </UFormGroup>
            </div>

            <div class="pt-6 border-t border-gray-200 dark:border-gray-800 flex justify-end">
              <UButton :loading="uploading" color="primary" class="font-black uppercase tracking-widest text-xs px-6 py-2" @click="uploadMappedFile">
                Merge Data
              </UButton>
            </div>
          </div>
        ...
        <UAlert v-if="errorMsg" color="red" variant="soft" icon="i-heroicons-x-circle" :title="errorMsg" />
        <UAlert v-if="successMsg" color="green" variant="soft" icon="i-heroicons-check-circle" :title="successMsg" />
      </div>
    </template>
  </UDashboardPanel>
</template>
    ...
    complete: (results) => {
      if (results.meta && results.meta.fields) {
        ...
        for (const field of standardFields) {
          const guess = csvColumns.value.find(c => c.toLowerCase().includes(field.id.toLowerCase()) || field.label.toLowerCase().includes(c.toLowerCase()))
...
const uploadMappedFile = async () => {
  ...
  try {
    ...
    const response = await $fetch<{ success: boolean; workoutId?: string }>('/api/workouts/upload-csv', {
      method: 'POST',
      body: formData
    })
    
    if (response.success) {
      successMsg.value = 'CSV successfully ingested! If this overlaps with an existing Garmin/Strava workout, the metrics will be merged automatically in the background.'
      setTimeout(() => {
        reset()
      }, 4000)
    }
